    ...
    <div className="overflow-hidden bg-gray-800 dark:bg-neutral-950">
      <div className="relative max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review) => (
            <div key={review.id} className="flex h-auto">
        ...
        <div className="mt-20 grid gap-6 grid-cols-2 sm:gap-12 lg:grid-cols-3 lg:gap-8">
          {stats.map((stat) => (
            <div key={stat.id}>
              <h4 className="text-lg sm:text-xl font-semibold text-white">{stat.label}</h4>
              <p className="mt-2 sm:mt-3 text-4xl sm:text-6xl font-bold text-blue-500">
                <CountUp end={stat.value} duration={2.5} />{stat.unit}
              </p>
              <p className="mt-1 text-gray-400">{stat.description}</p>
            </div>
          ))}
        </div>
        {/* End Grid */}
